...
The repository will become a small Bun workspace: `packages/messages` will publish `pronto-imessage`, and `packages/cli` will build the `pronto` executable by consuming that package's public interface. That restructuring follows the product rename so each migration remains independently testable. The standalone product becomes self-hosting before external consumers adopt the module. Consumers pin exact released versions even though the package follows semantic versioning.
